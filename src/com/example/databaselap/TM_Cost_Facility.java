...
public class TM_Cost_Facility {
	...
	private static final String CREATE_TABLE = 
			"create table "+NAMA_TABEL+" ( "+ROW_FACILITY_COST_ID+" integer PRIMARY KEY autoincrement, "+ROW_FACILITY_ID+" text, "+	
					" "+ROW_FACILITY_BRAND_NAME+" varchar, "+ROW_FACILITY_PRICE+" text, "+ROW_CREATED_BY+" date, " +	
	...
	private static class DatabaseOpenHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			final String[] fasilitas_susu = { "Bebelac 4-Madu-800 gr",
					...
					"Ensure-FOS-Chocolate-1000 gr", "Frisian-Flag-123-800 gr",
			...
			for (String facility : fasilitas) {
				if (facility.equals("1")) {
					for (String susu : fasilitas_susu) {
						values.put(ROW_FACILITY_ID, "1");
						values.put(ROW_FACILITY_BRAND_NAME, susu);
					...
					for (String vit : fasilitas_vitamin) {
						values.put(ROW_FACILITY_ID, "2");
						values.put(ROW_FACILITY_BRAND_NAME, vit);
						values.put(ROW_FACILITY_PRICE, "0");
						values.put(ROW_CREATED_BY, "-");
						values.put(ROW_CREATED_TIME, "-");
						values.put(ROW_UPDATE_BY, "-");
					...
					for (String popok : fasilitas_popok) {
						values.put(ROW_FACILITY_ID, "3");
						values.put(ROW_FACILITY_BRAND_NAME, popok);
						values.put(ROW_FACILITY_PRICE, "0");
						values.put(ROW_CREATED_BY, "-");
						values.put(ROW_CREATED_TIME, "-");
						values.put(ROW_UPDATE_BY, "-");
	...
	public CharSequence[] getAllBrandName(String id_facility) {
		...
		try{
		Cursor mCursor = db.rawQuery("SELECT  facility_brand_name  FROM  TM_Cost_Facility WHERE facility_id= '"+ id_facility + "'", null);
		if (mCursor != null) {
			mCursor.moveToFirst();
...
//
//			String sql_insert = "INSERT INTO TM_Cost_Facility(facility_id,facility_brand_name,facility_price,created_by,created_time,update_by,update_time) VALUES (?,?,?,null,null,null,null)";
//			for (String aa : data) {
